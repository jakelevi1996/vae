...
\begin{frame}[t]
    ...
    \begin{itemize}
        ...
        \begin{align}
            ...
            q(z_n \,\vert\, x_n, \phi) &= \mathcal{N}\Bigl( z_n \Bigm\vert \mu, \text{diag}(\sigma)^2 \Bigr) \tag{Encoder distribution}
        \end{align}
        \item[$\Rightarrow$] ``Reparameterise'' $z_n$ as transform of auxiliary noise variable $\varepsilon$:
        \begin{align*}
...
\begin{frame}
    ...
    \begin{align*}
        ...
        &\qquad + \textcolor{OliveGreen}{\frac{1}{L}\sum_{l=1}^L\Biggl[ \log \biggl(p\Bigl(x_n \Bigm\vert \mu + \varepsilon^{(l)} \odot \sigma, \theta\Bigr)\biggr) \Biggr]} \tag{Estimator} \\
        \\
        \mathbb{E}\Bigl[ \widehat{\mathcal{L}}(x_n, \theta, \phi) \Bigr] &= \mathcal{L}(x_n, \theta, \phi) \tag{Unbiased}
    \end{align*}
\end{frame}
...
\begin{frame}
    ...
    \begin{align*}
        (\mu, \sigma) &= \text{MLP}_\text{enc}(x_n, \phi) \tag{Encoder computation} \\
        \varepsilon^{(l)} &\sim \mathcal{N}(\varepsilon^{(l)} \vert 0, I) \tag{Auxiliary noise samples} \\
            z_n^{(l)} &= \mu + \varepsilon^{(l)} \odot \sigma \tag{Latent posterior samples} \\
        \widehat{\mathcal{L}}(x_n, \theta, \phi) &= \textcolor{blue}{\frac{1}{2}\sum_i \Bigl[ 1 + 2\log(\sigma_i) - \mu_j^2 - \sigma_j^2 \Bigr]} \\
        &\qquad + \textcolor{OliveGreen}{\frac{1}{L}\sum_{l=1}^L\Biggl[ \log \Bigl(\underbrace{p(x_n \vert z_n, \theta)}_{\text{MLP}_\text{dec}}\Bigr) \Biggr]} \tag{$\widehat{\text{ELBO}}$} \\
...
\begin{frame}
    ...
    \begin{align*}
        ...
        \\
        x&\sim \underbrace{p(x_n \vert z_n, \theta)}_{\text{MLP}_\text{dec}} \tag{Decode}
    \end{align*}
\end{frame}
