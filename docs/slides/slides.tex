...
\begin{frame}[t]
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            p(x_n \,\vert\, \theta) &= \sum_{z_n} \Bigl[ p\bigl(x_n, z_n \,\vert\, \theta\bigr) \Bigr] \tag{Latent variables} \\
...
\begin{frame}
    \frametitle{Evaluating reconstruction term}
    \begin{align*}
        \log\Bigl( p(x_n \,\vert\, z_n, \theta) \Bigr) &= \log\Biggl(\mathcal{N}\left( x_n \;\middle\vert\; \mu^{(x)}, \text{diag}\left(\sigma^{(x)}\right)^2 \right) \tag{Decoder log likelihood}\Biggr) \\
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Results: \url{https://github.com/jakelevi1996/vae/} (I know what you're thinking) \pause
        \item NB for MNIST:
        \begin{itemize}
            \vspace{1em}
            \setlength{\itemsep}{1em}
            \item Above demo = 100 epochs $\approx$ 30 minutes
            ...
            \item CF discriminative VS generative in general?
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{enumerate}
        \setlength{\itemsep}{2em}
        \item Conditional generation \cite{sohn2015learning}
        \item Molecule generation \cite{gomez2018automatic}
