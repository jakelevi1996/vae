...
\begin{frame}[t]
    ...
    \begin{itemize}
        ...
        \item Observe data $\{x_1, \dots, x_N\}$
        \item Assume $\{x_1, \dots, x_N\}\sim p(x | \theta^*)$
        \item[$\bigstar$] Want to sample $\{x_{N+1}, \dots, x_{N+M}\}\sim p(x | \theta^*)$
        \item Assume IID generative model with latent variables $z_n$:
        \begin{align*}
            \log\Bigl( p\bigl(x \,\vert\, \theta\bigr) \Bigr) &= \sum_{n=1}^N \left[ \log\Bigl( p\bigl(x_n \,\vert\, \theta\bigr) \Bigr) \right] \tag{IID} \\
            p(x_n \,\vert\, \theta) &= \sum_{z_n} \Bigl[ p\bigl(x_n, z_n \,\vert\, \theta\bigr) \Bigr] \tag{Marginalise} \\
